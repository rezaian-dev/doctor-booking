...
const DoctorProfile = ({ mode }: ProfileProps) => {
  ...
  const generateStars = (rating: number) => Array.from({ length: 5 }, (_, i) => i < rating);
  ...
  return (
    <>
      ...
      <div className="relative bg-white rounded-[10px] border overflow-hidden sm:overflow-visible border-neutral-100">
        ...
        <div className={clsx(mode === 'payment' && 'xs:p-4 sm:p-5', (mode ==="default" || mode==="confirm") && "xs:p-3 xs:pb-[9px]")}>
          <div className="flex xs:flex-row flex-col">
            ...
            <div className="flex items-start justify-between grow px-3 pt-3">
              <div className="flex flex-col gap-y-3">
                <h1 className="text-black font-medium text-lg">{doctorData.name}</h1>
                <span className="text-neutral-950 text-sm font-medium">{doctorData.specialty}</span>
              ...
              <div className="hidden lg:flex items-center gap-x-2">
                ...
                <span className="text-[13px] text-medium-gray">کد نظام پزشکی: {doctorData.medicalCode}</span>
              </div>
            </div>
          </div>

          {/* 📍 Additional info section — ONLY in default mode */}
          {(mode === 'default' || mode==="confirm") && (
            <div className="mt-3 xs:mt-2.5 px-3 xs:px-0 sm:px-0 pb-3 xs:pb-0">
              ...
              <div className="flex lg:hidden items-center gap-x-2 sm:gap-x-4 mb-2">
                ...
                <span className="text-xs xs:text-sm text-medium-gray">کد نظام پزشکی: {doctorData.medicalCode}</span>
              </div>

              {/* 📍 Address & Next Slot — Default mode only */}
              <div className="space-y-2">
                <div className="flex items-center gap-x-1 xs:gap-x-2">
                  <HugeiconsIcon icon={MapPinpoint02Icon} size={20} color="#262626" />
                  <span className="font-medium text-xs xs:text-sm text-neutral-950 whitespace-nowrap">آدرس مطب:</span>
                  <span className="text-neutral-850 text-xs xs:text-sm font-medium line-clamp-1">{doctorData.address}</span>
                </div>

                <div className="flex items-center gap-x-1 xs:gap-x-2">
                  <HugeiconsIcon icon={AlarmClockIcon} size={20} color="#262626" />
                  <span className="font-medium text-xs xs:text-sm text-neutral-950 whitespace-nowrap">اولین نوبت در دسترس:</span>
                  <span className="text-neutral-850 text-xs xs:text-sm font-medium line-clamp-1">{doctorData.nextAvailableSlot}</span>
                </div>
              </div>
            </div>
          )}
            ...
            <div className="mt-3 xs:mt-[18px] px-3 xs:px-0 sm:px-0 pb-3 xs:pb-0">
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center  px-2 rounded-[6px] ">
                  <span className="font-medium text-xs xs:text-sm text-neutral-600">آدرس مطب:</span>
                  <span className="text-neutral-850 text-xs xs:text-sm font-medium">{doctorData.address}</span>
                </div>

                <div className="flex justify-between items-center  px-2 rounded-[6px] ">
                  <span className="font-medium text-xs xs:text-sm text-neutral-600">نوع نوبت:</span>
                  <span className="text-neutral-850 text-xs xs:text-sm font-medium">حضوری</span>
                </div>

                <div className="flex justify-between items-center  px-2 rounded-[6px] ">
                  <span className="font-medium text-xs xs:text-sm text-neutral-600">زمان نوبت:</span>
                  <span className="text-neutral-850 text-xs xs:text-sm font-medium">دوشنبه ۲۴ دی ساعت ۹:۳۰</span>
                </div>

                <div className="flex justify-between items-center  px-2 rounded-[6px] ">
                  <span className="font-medium text-xs xs:text-sm text-neutral-600">مراجعه کننده:</span>
                  <span className="text-neutral-850 text-xs xs:text-sm font-medium">علی مهدوی</span>
                </div>
              </div>
            </div>
          )}
